...
function PricingJapanSection() {
  const [activeType, setActiveType] = useState('subscription')
  const activePlan = useMemo(() => planTypesJa[activeType], [activeType])
  ...
  return (
    <section id="pricing" className="px-4 py-14 sm:px-8 sm:py-20 lg:px-16">
      <div className="mx-auto w-full max-w-[1680px]">
          ...
          <button
            ...
            className={`cursor-pointer rounded-md border px-7 py-3 text-[14px] font-semibold transition-all duration-300 ease-[cubic-bezier(0.445,0.05,0.55,0.95)] active:scale-[0.99] sm:min-w-[220px] ${
              activeType === 'subscription'
                ? 'border-[#3f4c66] bg-white text-[#1f2128]'
                : 'border-[#a9b2bf] bg-[#f8f8f8] text-[#475063] hover:bg-white'
            }`}
          >
            サブスクプラン
          </button>
        ...
        <p className="mt-4 max-w-6xl text-[13px] leading-[1.7] text-[#2f3138] sm:text-[14px]">{activePlan.description}</p>

        <div className="mx-auto mt-7 w-full lg:w-10/12">
          <div className="grid gap-4 lg:grid-cols-2">
            {activePlan.cards.map((card) => (
              <article key={card.name} className="rounded-md border border-[#cfd5dc] bg-[#f6f7f8]">
                <div className="p-5 sm:p-6">
                  ...
                  <button
                    type="button"
                    className="mt-4 w-full rounded bg-[#1f8d57] px-5 py-2.5 text-[14px] font-bold text-white transition-all duration-300 ease-[cubic-bezier(0.445,0.05,0.55,0.95)] hover:bg-[#167346] hover:shadow-md active:scale-[0.99]"
                  >
                    プランを購入する
                  </button>
                  ...
                  <ul className="mt-5 space-y-2">
                    {card.items.map((item) => (
                      <li key={item} className="flex items-start gap-2 text-[13px] leading-[1.55] text-[#4a4a4a] sm:text-[14px]">
                        <span className="mt-0.5 text-[16px] font-bold text-[#10883a]">✓</span>
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
...
export default PricingJapanSection
