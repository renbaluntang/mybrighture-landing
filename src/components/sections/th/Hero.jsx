...
const ROTATING_WORDS = ['Clear', 'Confident', 'Effective']
    ...
    <section id="hero" className="relative w-full overflow-hidden bg-[#eceaea]">
      <div className="relative mx-auto grid w-full max-w-[1280px] items-center gap-10 px-4 pb-10 pt-8 sm:px-8 lg:grid-cols-2 lg:gap-12 lg:px-16 lg:pb-14 lg:pt-12">
        <div>
          ...
          <h1 className="mt-5 text-[2.2rem] font-extrabold leading-[1.03] tracking-[-0.02em] text-[#1a2340] sm:text-[3rem] lg:text-[3.6rem]">
            ...
            <span className="text-[#f15a22]">
              {displayed}
              <span className="ml-0.5 inline-block h-[0.88em] w-[3px] animate-[hero-blink_0.8s_step-end_infinite] bg-[#f15a22] align-text-bottom" />
            </span>
            <br />
            communication.
          </h1>
          ...
          <p className="mt-6 max-w-[520px] text-[1rem] leading-[1.75] text-[#4a5568] sm:text-[1.05rem]">
            Receive structured guidance and precise feedback that help you speak more accurately, naturally, and effectively in real-world conversations.
          </p>
          ...
          <div className="mt-9 grid max-w-[560px] grid-cols-3 gap-5 border-t border-[#d8dde7] pt-6">
            {[
              ...
              <div key={item.label}>
                ...
                <p className="mt-1 text-[0.68rem] font-semibold uppercase tracking-[0.08em] text-[#718096]">
                  {item.label}
                </p>
              </div>
            ))}
        ...
        <div className="relative">
          ...
          <div className="absolute -bottom-4 -left-4 animate-[hero-float_3s_ease-in-out_infinite] rounded-xl bg-white px-3 py-2 shadow-[0_10px_24px_rgba(0,0,0,0.18)] sm:px-4 sm:py-2.5">
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full bg-[#22c55e] animate-[hero-pulse_1.8s_ease-in-out_infinite]" />
              <span className="text-[0.72rem] font-semibold text-[#1a2340] sm:text-[0.78rem]">Live Class in Progress</span>
            </div>
          </div>

          <div className="absolute -right-3 -top-4 animate-[hero-float_3s_ease-in-out_infinite_0.4s] rounded-xl bg-white px-3 py-2 shadow-[0_10px_24px_rgba(0,0,0,0.18)] sm:px-4 sm:py-2.5">
            <div className="flex items-center gap-2">
      ...
      <style>{`
        ...
        @keyframes hero-float { 0%,100% { transform: translateY(0px); } 50% { transform: translateY(-8px); } }
        @keyframes hero-pulse { 0%,100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.45); opacity: 0.6; } }
      `}</style>
    </section>
  )
}
